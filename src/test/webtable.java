...
System.setProperty("webdriver.chrome.driver", "chromedriver.exe");
		...
		List<WebElement> Rows = driver.findElements(By.xpath("//table[@id='customers']/tbody/tr")); // to find rows:tr
		System.out.println("Total num of rows are " + Rows.size());
		
		
		List<WebElement> Columns = driver.findElements(By.xpath("//table[@id='customers']//th")); // to find header
		System.out.println("Total num of columns are " + Columns.size());
		
		WebElement Data = driver.findElement(By.xpath("//table[@id='customers']/tbody/tr[4]/td[1]")); // t find 4th row
		...
		for(int row=1; row<=Rows.size();row++) {
			...
			String Xpath = "//td[text()='" + Company + "']/following-sibling::td[2]";// we need 2nd sibling
